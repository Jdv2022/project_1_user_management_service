...
use App\Grpc\Handlers\SuggestedMemberDepartmentHandler;
...
class SuggestedMemberDepartmentTest extends TestCase {
	use RefreshDatabase;
	...
	public function test_suggested_member_department(): void {
		Log::info('test_suggested_member_department');	

		$suggestedMemberDepartmentHandler = new SuggestedMemberDepartmentHandler(new CommonFunctions());
		$in = new SuggestedMemberDepartmentRequest();
		$ctx = $this->createMock(ContextInterface::class);
		$result = $suggestedMemberDepartmentHandler->SuggestedMemberDepartment($ctx, $in);

		$this->assertInstanceOf(SuggestedMemberDepartmentResponse::class, $result);
