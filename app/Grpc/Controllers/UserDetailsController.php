...
class UserDetailsController implements GetUserDetailsServiceInterface {
	...
	public function GetUserDetails(ContextInterface $ctx, GetUserDetailsRequest $in): GetUserDetailsResponse {
		...
		$responseModel = new GetUserDetailsResponse();
		if($userDetailsObj) {
			...
			$responseModel->setUserDetailsCountry($userDetailsObj->country);
			$responseModel->setUserDetailsDateOfBirth($userDetailsObj->date_of_birth);
			$responseModel->setUserDetailsAge($userDetailsObj->age);
			...
			if($userDetailsObj->userRoles[0]->pivot) {
